...
def index(request):
    if request.method == 'GET':
        ...
        for c_sys in c_sysnams:
            syslist[ my_utils.judge_type(c_sys['sysnam']) ].append(
        ...
        newjobs = []
        for jt in jts:
            newjobs.append({'sysnam': jt['sysnam'], 'num': jt['num'], 'type': my_utils.judge_type(jt['sysnam'])})
...
def trend(request):
    ...
    for jt in jts:
        type_index = my_utils.judge_type(jt['sysnam'])  # 0 is for admin_data, 1 is for tech_data
        names = my_utils.split_sysnam(jt['sysnam'])
        for name in names:
            found = False
